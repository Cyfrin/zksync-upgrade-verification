This section will tell you how to manually do everything we cover here.
...
Click [this link](https://remix.ethereum.org/#url=https://github.com/Cyfrin/zksync-upgrade-verification/src/ExampleUpgradeCheckerFlat.sol) which will load `ExampleUpgradeCheckerFlat.sol` into remix.
...
In that transaction, you'll get a proposal ID under the `ProposalCreated` log. For ZIP4 example:
...
We can see the calldata here. 
You can also get this data from the transaction, with:

```
cast calldata-decode "propose(address[],uint256[],bytes[],string)" <PROPOSAL_CALLDATA>
```

And then take the `bytes[]` output into:
```
cast calldata-decode "sendToL1(bytes)" <SENDTOL1_CALLDATA>
```

All we need to do then, is decode each with `cast decode-calldata "execute(((address,uint256,bytes)[],address,bytes32))"` and the calldata. EXCEPT, we have to remember to append `a1dcb9b8`, since that's the function selector for the `execute` function.
...
Essentially, all you have to to is verify that the calldata you verified from the pre-vote section matches the calldata on the Ethereum chain.

To do this, look at the `test` repo. Essentially, you bundle up the transactions a called from ZKsync with `sendToL1(bytes)`, in the format on something like `ExampleUpgradeCheckerFlat.sol` looks like, and call the `getHash` on that contract. 